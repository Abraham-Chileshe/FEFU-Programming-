...
int main()
{
    ...
    while (fscanf(input_file, "%d %d", &num_teams, &num_data) != EOF)
    {
        ...
        for (int i = 0; i < num_data; ++i)
            fscanf(input_file, "%d %d %d %d", &data[i].team_number, &data[i].problem_number, &data[i].time, &data[i].result);
        ...
        for (int i = 1; i <= num_teams; ++i)
        {
            fprintf(output_file, i == 1 ? "%d" : " %d", teams[i].team_number);
        }
        fprintf(output_file, "\n");
    }
